...
class BotBase:
    ...
    def set(self,idbot,symbol,idinterval):

        self.idbot = idbot
        self.symbol = symbol
        self.idinterval = idinterval
        ...
        self.qty_dec_price = symbol_info['qty_dec_price']
        self.qty_dec_qty = symbol_info['qty_dec_qty']
    
    def create_order(self,symbol,side,type,quantity):
        new_order = pd.DataFrame([[1,self.base_asset,self.quote_asset]],columns=['idbot','base_asset','quote_asset'])
        try:
            ...
            executedPrice = round(float(order['cummulativeQuoteQty'])/float(order['executedQty']),self.qty_dec_price)
            qty = round(float(order['executedQty']),self.qty_dec_qty)
            new_order['orderId'] = order['orderId']
            new_order['price'] = executedPrice
            new_order['origQty'] = qty
    ...
    def run():
        ...
        if kline.update(self.SYMBOL):
            ...
            #            comprado_qty = comprado_qty + open_orders.loc[i]['qty']
            #            comprado_quote = comprado_quote + open_orders.loc[i]['qty'] * open_orders.loc[i]['price']
            ...
            if base_balance == 0 and signal == 'COMPRA':
                ...
                qty = round(self.QUOTE_TO_BUY/price,symbol_info['qty_dec_qty'])
            ...
            #Si esta comprado y hay señal de venta
                ...
                #Calcula los parametros de la orden
                qty = round((base_balance),symbol_info['qty_dec_qty'])
